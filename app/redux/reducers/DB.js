...
const default_data = Immutable.Map({
				...
				user: Immutable.Map({}),
				config: Immutable.Map({event_layout: 'list'}),
				...
				discountmaker: Immutable.Map({visible: false, closed: false, text: '', processing: false}),
				...
				tabs: Immutable.Map({user_messages: 'inbox'}),
				loaded: Immutable.Map({user_messages: false, review_data: false}),
				loading: Immutable.Map({user_messages: false, review_data: false}),

				verify_phone: Immutable.Map({requested: false, modal: false, processing: false}),
...
function DB(db = default_data, action) {
				...
				if (action.type === UPDATE_FIELD) {
								return db.setIn([
												action.payload.section, action.payload.key
								...
								// let messages = db.get('messages'); messages.push(
								// Immutable.Map(action.payload) ); return db.get('messages').push(
								// Immutable.Map(action.payload) ); let messages = db.get('messages').push(
								// Immutable.Map(action.payload) ); console.log('messages',messages); return
