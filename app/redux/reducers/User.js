...
function user(user = user_default_data, action) {
    ...
    if (action.type === USER_UPDATE_PROFILE) {
        if (action.payload && action.payload.gender) {
            action.payload.gender = action.payload.gender.toString();
        }
        return user.set('profile', Immutable.fromJS({...user.get('profile').toObject(),
            ...action.payload
        }));
    }
    if (action.type === USER_UPDATE_PROFILE_FIELD) {
        return user.setIn(['profile', action.payload.key], action.payload.value);
    }
    ...
    if (action.type === USER_CREATE_REGISTRATION_MANY) {
        ...
        let registrationIndex = user.get('registration').findIndex(item => {
            return item.id == action.payload.booking;
        });
        let updatedRegistration = user.get('registration').update(registrationIndex, (event) => {
