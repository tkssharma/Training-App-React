...
let WWWHeader = ( props ) => {
	...
	const popoverContent = (
		<div className="dropdown_container">
			...
			{ props.user.get('userType') == 2 ?
				TrainerNavigation : UserNavigation
			}
		</div>
	);
	let user_profile = () => {
    if(props.user.get('userType') === 1 || props.user.get('userType') === 2 ){
       ...
       return (<Link to="/auth/login">SignIn</Link>);
     }
      ...
      <nav role="navigation" className="navbar header-navigation-holder">
          <div className="container">
              <div className="navbar-header">
                  <button data-target="#header-navigation" data-toggle="collapse" className="navbar-toggle header-navigation-toggle" type="button">
                      <i className="icon-reorder"></i>
                  </button>
                  <Link to="/" href="" className="navbar-brand logo"><img alt="Logo with text" className="img-responsive" src="../images/logo.jpg"/></Link>
              </div>

              <div id="header-navigation" className="collapse navbar-collapse header-navigation">
	...
	return (
		<div>
      { props.user.get('userType') == 2 ? UserNavigation : UserNavigation
      }
		</div>
	);
}
...
const ConnectWWWHeader = connect(
	mapStateToProps,
	mapDispatchToProps
)(WWWHeader)
