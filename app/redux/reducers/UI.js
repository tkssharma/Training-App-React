...
function UIStore(ui = ui_default_data, action) {
    ...
    if (action.type === AUTH_UPDATE_USER_DATA) {
        if (action.payload && !action.payload.hasPassword) {
            return ui.setIn(['modals', 'update_password'], true);
        }
        return ui;
    } else if (action.type === UI_MODALS_UPDATE_FIELD) {
        return ui.setIn(['modals', action.payload.key], action.payload.value);
    } else if (action.type === UI_PROCESSING_UPDATE_FIELD) {
        return ui.setIn(['processing', action.payload.key], action.payload.value);
    } else if (action.type === UI_LOADED_UPDATE_FIELD) {
        return ui.setIn(['loaded', action.payload.key], action.payload.value);
    } else {
        return ui;
    }
